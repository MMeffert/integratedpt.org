...
- **Pages**: Each HTML page duplicates the full header/nav/footer structure (no templating). Each page has unique title, meta description, and keywords.
...
- `llms.txt` -- AI-readable plain-text business summary (services, locations, hours, contact).
...
CloudFront function `integratedpt-www-redirect` handles URL normalization:
- `www.integratedpt.org` → 301 → `integratedpt.org`
- `/index.html` → 301 → `/`
- Strips query strings from static pages
